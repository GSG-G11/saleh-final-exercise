...
const renderResulSummary = ( res, tags )=>{
  document.querySelector('#results-summary').innerHTML = ''
  + '<p>'
  + 'Query of ' + tags +  ' returned ' + res.items.length + ' results'
  + '</p>';
}
const renderResultBodyAns = (res , tags)=>{
  renderResulSummary(res , tags);
  document.querySelector('#results-body').innerHTML = res.items.map(function (item) {
    return ''
      + '<div>'
...
const renderResultBody = (res , tags)=>{
  renderResulSummary(res, tags);
  document.querySelector('#results-body').innerHTML = res.items.map(function (item) {
    return ''
      + '<div>'
...
document.querySelector('#form-unanswered').addEventListener('submit', function (e) {
  ...
  MyFetch(url , tags , renderResultBody);
});
...
document.querySelector('#form-answerers').addEventListener('submit', function (e) {
  ...
  MyFetch(url , tag , renderResultBodyAns);
});
